...
db.addToDb = (str, rec, cb) => {
	...
	longUrl = longUrl.replace(/http(s)?:\/\//, '').replace(/[\/\.]/g, '_');
	...
	fs.writeFile(filepath2, rec1, 'utf8', (err) => {
		if (err) {
			cb(err);
			return;
		}
		fs.writeFile(filepath1, rec2, 'utf8', (err) => {
			if (err) {
				cb(err);
				return;
			}
...
db.addToDb('INSERT INTO urls SET ?', 
	{longUrl: 'yandex.ru', shortUrl: 'aaa0009'}, 
	console.log);
